...
def getLabel(anno_path, data, img_path, cat=0):
    img_name = "{}".format(str(data[0]).zfill(6))
...
def generate_prediction(count_data, file_names):
    ...
    for cate in pascal_class_dic:
        ...
        for line, filename in zip(count_data, file_names):
            ...
            #in the ground truth, this file not include the item in this category, then just skipp
            file_count += 1
            ...
            box_numbers = 147
            this_file_score_list = operator.itemgetter(range(file_count*box_numbers,file_count*box_numbers+box_numbers))(data)
            ...
            global_grids = [x+pascal_class_dic[cate]*49 for x in gt_local_grids]
...
def show_creteria(thresh):
    classes = 20
    grids = 7
    csv_file = "yolo_grid_thresh{}.csv".format(thresh)
    pred_img_names, labels = fromCsv(csv_file)
    mlb = MultiLabelBinarizer(range(classes*grids*grids))
    pred = mlb.fit_transform(labels).astype(np.float32)

    gt_image_names, gt_labels = fromCsv("test_grid_voc2007_grid_7.csv")
...
if __name__ == '__main__':
    ...
    generate_prediction(counts, pred_img_names)
